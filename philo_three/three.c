...
/*   Updated: 2020/09/19 15:00:49 by ckakuna          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		main(int ac, char **av)
{
	...
	if ((status = start_process(&argv)) != 0)
		return (print_error("Invalid start process\n", status));
	sem_wait(argv.who_dead);
	clear_leaks(&argv);
	return (status);
}
